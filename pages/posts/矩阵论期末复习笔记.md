---
title: 矩阵论期末复习笔记
description: 矩阵论期末复习笔记：线性空间、矩阵分解、特征值与 Jordan 标准形
tags:
  - Course
categories: Math
date: 2026-03-09
...
这篇是来自[limfx](https://www.limfx.pro/readarticle/1197/ju-zhen-lun-fu-xi-bi-ji)整理的矩阵论期末复习笔记，保留原有知识点编排，补齐文章结构，便于后续查阅。
...
*   $W_1$与$W_2$的**和空间**为：$W_1 + W_2 = \{ \alpha | \alpha = \alpha_1 + \alpha_2, \; \alpha_1 \in W_1, \; \alpha_2 \in W_2 \}$
...
*   $R^2$上**绕原点逆时针旋转**$\theta$**角的线性变换**$T_{\theta}$**称为**_**正交变换**_，在标准正交基下对应的变换矩阵$\begin{bmatrix} \cos \theta &  - \sin \theta \\ \sin \theta & \cos \theta \end{bmatrix}$是**正交矩阵**。

*   空间$R^3$上**绕过原点的直线**$l$**旋转**$\theta$**角的变化**$T_{L_{\theta}}$**为**_**正交变换**_，在标准正交基下对应的变换矩阵$\begin{bmatrix}1 &  & \\ & \cos \theta & -\sin \theta \\ & \sin \theta & \cos \theta \end{bmatrix}$是**正交矩阵**。
...
*   若$\lambda_i$是$k_i$重（_代数重数_）的，$\dim V_{\lambda_i}$是_几何重数_，则有$\dim V_{\lambda_i} \leqslant k_i$。
...
*   对$\lambda_i$，由$(A-\lambda_i I)X = 0$，求$A$的**线性无关的特征向量**$\alpha_1, \alpha_2, ..., \alpha_{t_i}$，其中$t_i$是$\lambda_i$的**几何重数，决定了Jordan块的个数**；
...
> **奇异值**：对$rank(A)=r$的矩阵$A$，矩阵$A^HA$的**非零特征值**有$\lambda_1 \geqslant \lambda_2 \geqslant ... \geqslant \lambda_r >0$，则称正数$\sigma_i = \sqrt{\lambda_i}$为矩阵$A$的奇异值。
...
（1）设$A \in C^{m \times n}, B \in C^{n \times m}$，若有$BA=I_n$，则称$B$是$A$的一个**左逆**。
...
（2）设$A \in C^{m \times n}, C \in C^{n \times m}$，有$AC = I_m$，则称$C$是$A$的一个**右逆**。
...
*   **列和范数**： ${\color{red}{||A||_1 = \underset{j}{\max}(\sum_{i=1}^n |a_{ij}|)}}$**，即**每一列各元素模相加其中的最大值

*   **谱范数**：${\color{red}{||A||_2 = \sqrt{\lambda_1}}}$，其中$\lambda_1$是$A^HA$的**最大特征值**

*   **行和范数**：${\color{red}{||A||_{\infty} = \underset{i}{\max}(\sum_{j=1}^n |a_{ij}|)}}$，即**每一行各元素模相加其中的最大值**
...
> **定义**：$\lambda_1, \lambda_2, ..., \lambda_n$是矩阵$A \in C^{n \times n}$的全部特征值，称${\color{red}{\rho(A)=\underset{i}{\max}|\lambda_i|}}$**为**$A$**的**谱半径。
...
若复变量$z$的幂级数$\sum_{k=0}^{\infty}a_kz^k$的**收敛半径为**$R$，而方阵$A \in C^{n \times n}$的**谱半径为**$\rho(A)$，则
